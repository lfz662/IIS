...
angular.module("umbraco").controller("UmbracoForms.SettingTypes.RangeController",
    function ($scope, $element) {
        ...
        // The PreValues Settings is a string array in a certain order: min, max, step, default.
        vm.range.min = parseFloat($scope.setting.prevalues[0]);
        vm.range.max = parseFloat($scope.setting.prevalues[1]);
        vm.range.step = parseFloat($scope.setting.prevalues[2]);
        vm.range.default = parseFloat($scope.setting.prevalues[3]);

        // Set the provided default value if we don't have one already set.
        if (!$scope.setting.value) {
            $scope.setting.value = vm.range.default;
        }

        // We need to set the step directly on the element to avoid the issue described here:
        // https://stackoverflow.com/questions/26746163/angularjs-issue-with-binding-slider-input-to-decimal-value
        var rangeInput = $("input[type='range']", $element);
        rangeInput.each(function (idx, el) {
            $(el).attr("step", vm.range.step);
        });
...
(function () {
    ...
    function formDesignController($scope, $timeout, formResource, userService, securityResource) {
        ...
        formResource.getPrevalueSources().then(function (resp) {
            ...
            formResource.getAllFieldTypesWithSettings().then(function (resp) {
                ...
                // 200ms timeout as a timing/dom issue with the field labels etc
                $timeout(() => {
                    vm.ready = true;
                }, 200);
            });
...
angular.module("umbraco.directives")
    .directive('umbFormsDesignerNew', function (dialogService, formService, workflowResource, fieldResource, notificationsService) {
        return {
            scope: {
                form: "=",
                fieldtypes: "=",
            ...
            link: function (scope, element, attrs, ctrl) {
                ...
                scope.addField = function(fieldset, container) {
                    ...
                    scope.fieldSettingsOverlay = {
                        ...
                        submit: function (model) {
                            emptyField.settings = {};
                            validateSettings(model, emptyField);                            
                        },
                        close: function (oldModel) {                            
                ...
                scope.openFieldSettings = function(field){
                    ...
                    scope.fieldSettingsOverlay = {
                        ...
                        show: true,
                        submit: function (model) {
                            validateSettings(model, field);
                        },
                        close: function (oldModel) {
                            var fieldIndex = scope.fields.findIndex(function (p) {
                                return p.id === field.id;
                            });
                            if (fieldIndex > 0) { 
                                field.$fieldType = oldModel.fields[fieldIndex].$fieldType;                                
                            }                                                                                    
                            scope.fieldSettingsOverlay = null;
                ...
                var validateSettings = function (model, field) {
                    fieldResource.validateSettings(model.field).then(function (response) {
                        if (response.data.length > 0) {
                            angular.forEach(response.data, function (error) {
                                notificationsService.error("Field failed to submit ", error.Message);
                            });
                        } else {
                            field.settings = {};
